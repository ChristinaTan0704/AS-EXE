...
bool ConstraintTable::constrained(Path path)
{	
	int start_time = path.begin_time;
	for (int i = 0; i < path.size(); i++)
	{	
		// vertex contraint
		if (constrained(path[i].location, start_time + i))
			return true;
		
		// edge constraint
		if (i + 1 < path.size() && constrained(path[i].location, path[i + 1].location, start_time + i))
			return true;
	}
	return false;
}
...
void ConstraintTable::copy(const ConstraintTable &other)
{
	...
	for (const auto& entry : other.ct) {
		// cout << "copying ct " << entry.first  << " " << entry.second.size() << endl; // Debug TODO del
		ct[entry.first].clear();
		ct[entry.first].assign(entry.second.begin(), entry.second.end());
	}
	
	for (const auto& entry : other.landmarks) {
		landmarks[entry.first] = entry.second; // Deep copy of the list
	}
	...
	// ct.clear();
	// for (const auto& [key, value] : other.ct) {
	// 	ct[key] = value; // Deep copy of the list
	// }

	// landmarks.clear();
	// for (const auto& [key, value] : other.landmarks) {
	// 	landmarks[key] = value; // Deep copy of the list
	// }
	// landmarks = other.landmarks;
	// landmarks = other.landmarks
    // Manually copy ct (unordered_map)
    // ct.clear();
    // for (const auto &entry : other.ct) {
    //     ct[entry.first] = entry.second;  // Copy each pair
    // }

	// landmarks.clear();
    // for (const auto &entry : other.landmarks) {
    //     landmarks[entry.first] = entry.second;  // Copy each pair
    // }
	// landmarks = other.landmarks;
	// we do not copy cat
}
