...
public:
    ...
    bool SWITCH = true;
    bool SIMULATE = true; // consider the independent vertex after simulation
    int sys_timestep = 0;
    vector<bool> agent_free;
    vector<int> agent_carryingShelf;
    vector<int> agent_continueVertex;
    ...
    vector<vector<std::tuple<int, int, int, int>>> agent_currPath;
    vector<int> agent_currPathStep;
...
private:
    ...
    void Plan_path_to_loc(int agent, vector<int> longest_traj);
    ...
    bool Pass_all_vertex(){
        ...
        // }
        // passed 
        return std::none_of(passed.begin(), passed.end(), [](bool pass) { return pass == false; });
